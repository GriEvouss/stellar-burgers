...
describe('Тест экшенов конструктора', () => {
  // Удаляю общий startState

  describe('Тест экшена добавления ингредиента', () => {
    test('должен добавлять обычный ингредиент', () => {
    ...
    test('должен заменять булку', () => {
      ...
      const endState: constructorState = JSON.parse(JSON.stringify(startState));
      endState.constructorItems.bun = {
        ...bun,
        id: 'mockedID'
      };

      const newState = constructorReducer(startState, addIngredient(bun));
  ...
  describe('Тест экшенов изменения порядка ингредиентов', () => {
    ...
    test('не должен перемещать ингредиент вверх, если он первый', () => {
      const startState: constructorState = JSON.parse(
        JSON.stringify(constructorInitialState)
      );
      startState.constructorItems = {
    ...
    test('не должен перемещать ингредиент вниз, если он последний', () => {
      const startState: constructorState = JSON.parse(
        JSON.stringify(constructorInitialState)
      );
      startState.constructorItems = {
      ...
      const ingredientId = startState.constructorItems.ingredients.length - 1;
      const newState = constructorReducer(
        startState,
        moveIngredientDown(ingredientId)
      );
